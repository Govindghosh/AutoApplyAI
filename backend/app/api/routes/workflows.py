...
def serialize_workflow_details(workflow: ApplicationWorkflow, steps: list[WorkflowStep]):
    return {
        "workflow": {
            "id": workflow.id,
...
async def get_workflow_by_job(
    ...
    return serialize_workflow_details(workflow, get_workflow_steps(db, workflow.id))
